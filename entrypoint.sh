...
# Utilizing Gunicorn as a process manager with 3 Uvicorn async workers.
# This keeps the memory footprint low (~250-400MB) while multiplexing connections.
echo "Starting Gunicorn + Uvicorn..."
exec gunicorn rpm.asgi:application -k uvicorn.workers.UvicornWorker -w 3 --bind 0.0.0.0:8000
